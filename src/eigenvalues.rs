...
/// flags are enabled. If `lapack-backend` is enabled, then the default
/// eigenvalue backend is the LAPACK backend. Otherwise, if `faer-backend` is
/// enabled, the default eigenvalue backend is the faer backend.
